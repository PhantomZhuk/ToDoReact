...
function ToDoList({ todo, setToDo }: any) {
    ...
    return (
        <div className="w-full h-[80vh] pt-[30px] flex flex-wrap justify-center items-start gap-5 overflow-auto hide-scrollbar">
            {todo.map((item: any) => (
                <div key={item.id} className={`w-[350px] min-h-[200px] flex flex-col justify-between p-[10px] text-white bg-[#292c35] rounded-lg ${item.status ? `opacity-30` : `opacity-100` }`}>
                    ...
                    <div className="flex justify-center gap-5 mt-4">
                        ...
                        <button onClick={() => editTitle(item.id)} id={`edit-${item.id}`} className="px-[32px] py-[8px] bg-[#df9146] rounded-lg"><FontAwesomeIcon icon={faPenToSquare} /></button>
                        <button onClick={() => saveTitle(item.id, (document.getElementById(`title-${item.id}`) as HTMLParagraphElement).innerText)} id={`save-${item.id}`} className="px-[32px] py-[8px] bg-[#df9146] rounded-lg hidden"><FontAwesomeIcon icon={faCheck} /></button>
                    </div>
                </div>
            ))}
        </div>
